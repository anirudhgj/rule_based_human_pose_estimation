...
def embedding_branch(res4f, reuse=tf.AUTO_REUSE, drop1=1.0, drop2=1.0):
    with tf.variable_scope('embedding', reuse=reuse):
        ...
        fc_embedding_5 = tc.layers.fully_connected(fc_embedding_4, num_outputs=128,scope='fc_4')
        fc_embedding_6 = tc.layers.fully_connected(fc_embedding_5, num_outputs=128,scope='fc_5')
        act = lambda x: 10 * tf.tanh(x/10)
        fc_embedding = tc.layers.fully_connected(fc_embedding_6, num_outputs=32,scope='fc_6', activation_fn=act)
        return fc_embedding
