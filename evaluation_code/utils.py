...
def draw_limbs_3d_plt(joints_3d, ax, limb_parents=limb_parents, z_tilt=True):
    for i in range(joints_3d.shape[0]):
        ...
        if z_tilt:
            ax.plot(z_pair, x_pair, y_pair, linewidth=2, antialiased=True)
        else:
            ax.plot(x_pair, y_pair, z_pair, linewidth=2, antialiased=True)
...
def do_scatter(pose, ax, z_tilt=False):
    ...
    for i in range(pose.shape[0]):
        x,y,z = pose[i]
        if not z_tilt:
            ax.text(x,y,z, str(i),fontsize=6)
        else:
            ax.text(z,x,y, str(i),fontsize=6)
...
def gen_image_plot(fig,x_recon , gt , image,az=0, el=10):
    gs = gridspec.GridSpec(1,3, wspace=0.2, hspace=0.2)

    ax = fig.add_subplot(gs[0], projection='3d')
    lim = np.max(np.abs(x_recon))
    ax.view_init(azim=az, elev=el)
    ax.set_xlim(-lim, lim)
    ax.set_ylim(-lim, lim)
    ax.set_zlim(-lim, lim)
    ...
    ax.set_ylabel('y')
    ax.set_zlabel('z')
    plot_skeleton_and_scatter(gt, ax)
    ax.title.set_text('gt')


    ax = fig.add_subplot(gs[2])
    ax.imshow(image)
    ax.title.set_text('input_image')
...
def gen_image_plot4(fig,x_recon , gt , image,az, el):
    rows = 4
    cols = 3
    gs = gridspec.GridSpec(rows,cols, wspace=0.2, hspace=0.2)


    for i in range(0,rows):
        idx = i*(rows -1)
        ax = fig.add_subplot(gs[idx], projection='3d')
        lim = np.max(np.abs(x_recon[i]))
        ax.view_init(azim=az[i], elev=el[i])
        ax.set_xlim(-lim, lim)
        ax.set_ylim(-lim, lim)
        ax.set_zlim(-lim, lim)
        ax.set_xlabel('x')
        ax.set_ylabel('y')
        ax.set_zlabel('z')
        plot_skeleton_and_scatter(x_recon[i], ax)
        ax.title.set_text('pred')
    ...
    for i in range(0,rows):
        ...
        ax.set_ylabel('y')
        ax.set_zlabel('z')
        plot_skeleton_and_scatter(gt[i], ax)
        ax.title.set_text('gt')
...
#     thetas = np.random.uniform(z_limit[0],z_limit[1], pose_seq.shape[0])
    thetas = [z_limit[0]]*pose_seq.shape[0]
...
#     thetas = np.random.uniform(y_limit[0],y_limit[1], k.shape[0])
    thetas = [y_limit[0]]*pose_seq.shape[0]
