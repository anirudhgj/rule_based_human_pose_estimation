...
def embedding_branch(res4f, reuse=tf.AUTO_REUSE, drop1=1.0, drop2=1.0):
    with tf.variable_scope('embedding', reuse=reuse):
        ...
        fc_embedding_2 = tc.layers.fully_connected(fc_embedding_1, num_outputs=512,scope='fc_2')
        fc_embedding_3 = tc.layers.fully_connected(fc_embedding_2, num_outputs=256,scope='fc_3')
        fc_embedding_4 = tc.layers.fully_connected(fc_embedding_3, num_outputs=128,scope='fc_4')
        act = lambda x: 10 * tf.tanh(x/10)
        fc_embedding = tc.layers.fully_connected(fc_embedding_4, num_outputs=32,scope='fc_5', activation_fn=act)
        return fc_embedding, fc_embedding_1


def mlp_head(x, name, reuse = tf.AUTO_REUSE):
    with tf.variable_scope(name, reuse = reuse):
        x = tc.layers.fully_connected(x, 512)
        x = tc.layers.fully_connected(x, 256)
        output = tc.layers.fully_connected(x, num_outputs = 128)
    return output
